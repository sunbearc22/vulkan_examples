...
class Setup(object):
    ...
    def _createInstance(self):
        ...
        logging.debug('Initialised Vulkan Loader/Library')
        ...
        logging.debug('Created Vulkan Instance Create Info')
    ...
    def _getFnp(self):
        ...
        logging.info('Created function pointers:')
    ...
    def _setupSurface(self):
        ...
        def _surface_xlib():
            logging.debug('Created Xlib surface function.')
        ...
        def _surface_xcb():
            logging.debug('Created Xcb surface function')
        ...
        def _surface_wayland():
            logging.debug('Created Wayland surface function')
        ...
        def _surface_mir():
            logging.debug('Created Mir surface function')
        ...
        def _surface_win32():
            logging.debug('Created Win32 surface function')
    ...
    def _createSwapChain(self):
        ...
        if surface_capabilities.currentExtent.width != uint32_max:
            ...
            sc_imageExtent.width = self.window.w
            sc_imageExtent.height = self.window.h
            sc_imageExtent.width = max(
                self.capabilities.minImageExtent.width,
                min(self.capabilities.maxImageExtent.width, sc_imageExtent.width))
            sc_imageExtent.height = max(
                self.capabilities.minImageExtent.height,
                min(capabilities.maxImageExtent.height, sc_imageExtent.height))
        logging.info('Set swapchain extent.width = {}'.format(sc_imageExtent.width))
        logging.info('Set swapchain extent.height = {}'.format(sc_imageExtent.height))
        ...
        createInfo = VkSwapchainCreateInfoKHR (
            ...
            imageExtent = self.swapchain_imageExtent,
    ...
    def _createImageviews(self):
        ...
        #1. Get the array of presentable images in swapchain')
        swapchain_images = self.fnp['vkGetSwapchainImagesKHR'](
            self.logical_device, self.swapchain)
        logging.info('Obtained swapchain images.')

        #2. Create a view of each swapchain image and store in an array.
        try:
            for i, image in enumerate(swapchain_images):
                # Component specifies a remapping of color components (or of
                # depth or stencil components after they have been converted
                # into color components). Allows us to swizzle the color
                # channels around if need. To use the default color mapping,
                # use VK_COMPONENT_SWIZZLE_IDENTITY
                ...
                # subresourceRange describes what the image's purpose is and
                # which part of the image should be accessed. Our images will
                ...
                createInfo = VkImageViewCreateInfo(
                    ...
                    pNext = None,
                    flags = 0,
                    image = image, # the current VkImage on which the view will be created.
                    viewType = VK_IMAGE_VIEW_TYPE_2D, #treat images as 1D, 2D or 3D textures or cube maps.
            ...
            logging.info('Created {} Swapchain Image Views.'.format(
                len(self.swapchain_imageViews)))
    ...
    def _createGraphicsPipeline(self):
        ...
        scissor = VkRect2D(offset=scissor_offset, extent=self.swapchain_imageExtent)
    ...
    def _createCommandBuffer(self):
        ...
        try:
            for i, command_buffer in enumerate(self.command_buffers):
                ...
                vkBeginCommandBuffer(command_buffer, command_buffer_begin_createInfo)
    ...
    def cleanup(self):
        ...
        if self.graphics_pipeline:
            vkDestroyPipeline(self.logical_device, self.graphics_pipeline, None)
