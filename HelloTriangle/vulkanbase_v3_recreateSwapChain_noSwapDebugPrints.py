...
class Setup(object):
    ...
    def _createSwapChain(self):
        ...
        # bounds. If not, the swap extent must exactly match the surface 
        # current extent; this scenario is most common. The criterion
        ...
        # i.e. UINT32_MAX.
        #added: Get up-to-date window size during possible window resizing.
        ...
        if surface_capabilities.currentExtent.width != uint32_max:
            #Swapchain size must exactly match the surface size.
            sc_imageExtent = VkExtent2D( width=newWidth, height=newHeight )

        else:
            #Swapchain size does not need to match surface size.
            sc_imageExtent.width = max(
                surface_capabilities.minImageExtent.width,
                min(surface_capabilities.maxImageExtent.width, newWidth) ) #changed to actual extent

            sc_imageExtent.height = max(
                surface_capabilities.minImageExtent.height,
                min(surface_capabilities.maxImageExtent.height, newHeight) ) #changed to actual extent
        ...
        def _pickSurfaceFormat(formats):
            ...
            if len(formats)==1 and formats[0].format==VK_FORMAT_UNDEFINED:
                #Surface format is undefined.
                return VkSurfaceFormatKHR(VK_FORMAT_B8G8R8A8_UNORM, 0)

            #Surface format is defined. Use available format combination
            for f in formats:
                if f.format == VK_FORMAT_B8G8R8A8_UNORM and \
                   f.colorSpace == VK_COLOR_SPACE_SRGB_NONLINEAR_KHR:
                    return f

            #Use 1st detected surface format combination.
    ...
    def _drawFrame(self):
        ...
        try:
            ...
            logging.error(
                "Image in present queue VK_ERROR_OUT_OF_DATE_KHR:"
                " Recreating Swapchain.")
            self._recreateSwapChain()
        except VK_SUBOPTIMAL_KHR:
            logging.error(
                "Image in present queue VK_SUBOPTIMAL_KHR:"
                " Recreating swapchain.")
            self._recreateSwapChain()
        except VkError as e:
            print('VkError: {}'.format(e))
            print("Failed to present swapchain image!")
        except VkException as e:
            print('VkException: {}'.format(e))
            print("Failed to present swapchain image!")
    ...
    def _cleanupSwapChain(self):
        ...
        if self.command_pool:
            vkFreeCommandBuffers( self.logical_device, self.command_pool,
                                  len(self.command_buffers), self.command_buffers )
            #logging.info('Free existing Command Buffers.')
